...
export function cumulativeCashflows(cashflows) {
    const out = [];
    let s = 0;
...
export function simplePaybackYears(capex, annualNetSavings) {
    if (annualNetSavings <= 0)
        return Number.POSITIVE_INFINITY;
    return capex / annualNetSavings;
}
// IRR via bisection method
export function irr(cashflows, maxIter = 60) {
 ...
 */
export function lcoe(discountRate, costs, energy) {
 ...
 */
export function lcos(discountRate, costs, energyDelivered) {
 ...
 * costs: yearly costs
 */
export function benefitCostRatio(discountRate, benefits, costs) {
