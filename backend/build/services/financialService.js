...
export async function runCandidateGrid(reoptResult, budgetRange) {
    ...
    for (const pv of pvOptions) {
        for (const storage of storageOptions) {
            ...
            const storageCost = storage * ((reoptResult.ElectricStorage?.capital_cost_per_kwh ?? 500));
            const capex = pvCost + storageCost;
            if (capex >= budgetRange.min && capex <= budgetRange.max) {
                const annualNetSavings = (reoptResult.Financial?.elec_savings_annual ?? 0) * (pv / 5);
...
export async function runMonteCarlo(candidates) {
