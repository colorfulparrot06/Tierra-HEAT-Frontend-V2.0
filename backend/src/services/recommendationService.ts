...
export const recommendSystems = (geothermal: GeothermalData, solar: SolarData) => {
  ...
  if (ghi && ghi > 4.5) {
    recommendations.push(
      "High solar irradiance detected — rooftop solar PV systems are strongly recommended."
    );
  } else if (ghi) {
    recommendations.push(
      "Moderate solar resource — solar systems are viable with optimized tilt and tracking."
    );
  } else {
  ...
  if (geothermal.count > 0) {
    ...
    if (hasWeatherFile) {
      ...
      recommendations.push(
        "No specific geothermal components found nearby — feasibility study suggested."
      );
    }
  } else {
    recommendations.push(
      "No nearby geothermal resources found — rely primarily on solar systems."
    );
  }
