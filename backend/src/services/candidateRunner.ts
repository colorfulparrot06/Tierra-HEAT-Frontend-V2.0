...
type GridOptions = {
  ...
  pv_kw_grid?: number[]; // e.g. [0,50,100,200]
  storage_kwh_grid?: number[]; // e.g. [0,50,100]
  concurrency?: number;
  timeoutMs?: number;
};

export async function runCandidateGrid(opts: GridOptions) {
  ...
  const combinations: { pv: number; storage: number }[] = [];

  for (const pv of pvGrid) {
    for (const st of storageGrid) combinations.push({ pv, storage: st });
  }
  ...
  return results.map((r) => {
    if (r.error) return { id: `pv${r.combo.pv}_st${r.combo.storage}`, error: r.error };
...
export async function evaluateMonteCarlo(opts: {
  lat: number;
  lon: number;
  candidate: { pv_kw?: number; storage_kwh?: number } | any;
  loads_kw?: number[];
  doe_reference_name?: string;
  annual_kwh?: number;
  urdb_label?: string;
  ...
  samples?: number;
  uncertainty?: {
    capex_pct_std?: number; // e.g., 0.10 for 10% std
    elec_escalation_pct_std?: number;
    energy_prod_pct_std?: number;
  };
  timeoutMs?: number;
}) {
  ...
  for (let i = 0; i < samples; i++) {
    ...
    const cashflows: number[] = [-capexSample];
