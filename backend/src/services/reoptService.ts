...
export interface RunOptions {
  ...
  urdb_label?: string;
  analysis_years?: number;
  pv_existing_kw?: number;
  timeoutMs?: number;
  financial?: FinancialOptions;
  max_kw?: number; // limit PV considered (for candidate runs)
  max_kwh?: number; // limit storage considered
}
 ...
 */
export async function runReopt(lat: number, lon: number, opts: RunOptions = {}) {
  ...
  if (opts.financial) Object.assign(scenario.Financial, opts.financial);
  ...
  if (opts.loads_kw && opts.loads_kw.length > 0) {
    ...
    if (opts.annual_kwh) scenario.ElectricLoad.annual_kwh = opts.annual_kwh;
  } else {
  ...
  if (typeof opts.max_kw === "number") scenario.PV.max_kw = opts.max_kw;
  if (typeof opts.max_kwh === "number") scenario.ElectricStorage.max_kwh = opts.max_kwh;

  if (typeof opts.pv_existing_kw === "number") scenario.PV.existing_kw = opts.pv_existing_kw;
  ...
  if (!run_uuid) throw new Error("REopt API did not return run_uuid");
